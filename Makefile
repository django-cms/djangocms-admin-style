...
run111:
	...
	docker run -t --rm -p $(PORT):8000 -v `pwd`:/app djangocms-admin-style-test:django111 bash -c "pip3 install -r tests/requirements/django-1.11.txt && python3 tests/testserver.py"

run20:
	make build
	docker build -t djangocms-admin-style-test:django20 -f Dockerfile.django20 .
	docker run -t --rm -p $(PORT):8000 -v `pwd`:/app djangocms-admin-style-test:django20 bash -c "pip3 install -r tests/requirements/django-2.0.txt && python3 tests/testserver.py"

run21:
	make build
	docker build -t djangocms-admin-style-test:django21 -f Dockerfile.django21 .
	docker run -t --rm -p $(PORT):8000 -v `pwd`:/app djangocms-admin-style-test:django21 bash -c "pip3 install -r tests/requirements/django-2.1.txt && python3 tests/testserver.py"
