...
run20:
	...
	docker run -t --rm -p $(PORT):8000 -v `pwd`:/app djangocms-admin-style-test:django20 bash -c "pip install -r tests/requirements/django-2.0.txt && python tests/testserver.py"

run21:
	make build
	docker build -t djangocms-admin-style-test:django21 -f Dockerfile.django21 .
	docker run -t --rm -p $(PORT):8000 -v `pwd`:/app djangocms-admin-style-test:django21 bash -c "pip install -r tests/requirements/django-2.1.txt && python tests/testserver.py"
